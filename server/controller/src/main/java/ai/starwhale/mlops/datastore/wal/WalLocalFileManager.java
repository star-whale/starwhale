...
public class WalLocalFileManager {
    ...
    private final TreeMap<Long, LogFileInfo> logFileMap = new TreeMap<>();
    ...
    public LogFileInfo getLogFileInfo(long index) {
        var info = this.logFileMap.get(index);
        return info == null ? null : new LogFileInfo(info);
    }

    public List<LogFileInfo> getAllLogFileInfo() {
        return this.logFileMap.values().stream().map(LogFileInfo::new).collect(Collectors.toList());
    }

    public LogFileInfo getCurrentLogFileInfo() {
        return this.getLogFileInfo(this.logFileIndex);
    }
    ...
    public int write(List<WalEntry> entries) throws IOException {
        ...
        var info = this.logFileMap.get(this.logFileIndex);
        info.maxEntryId = entries.get(ret - 1).getId();
        info.size += sizeTotal;
        return ret;
    }

    public Iterator<WalEntry> readAll(int maxUncompressedSize) {
        return new WalEntryReader(this.indexes, maxUncompressedSize, true) {

            @Override
            public WalEntry next() {
                var entry = super.next();
                var info = WalLocalFileManager.this.logFileMap.computeIfAbsent(this.getCurrentLogIndex(),
                        LogFileInfo::new);
    ...
    private void initLogFile() {
        ...
        var info = new LogFileInfo(this.logFileIndex);
        this.logFileMap.put(this.logFileIndex, info);
        for (int i = 0; ; i++) {
            try {
                this.outputStream = Files.newOutputStream(info.getPath(),
                ...
                if (i % 100 == 0) {
                    log.error("failed to create new log file" + info.getPath(), e);
                }
                try {
