...
public class BatchOperateHelper {
     ...
     */
    public static <T, U> void doBatch(Collection<T> batchObject, U param, BiConsumer<Collection<T>, U> batchOperate,
     ...
     * split one giant batch to small batches
     */
    public static <T> void doBatch(Collection<T> batchObject, Consumer<Collection<T>> batchOperate,
            final Integer maxBatchSize) {
        if (batchObject.size() > maxBatchSize) {
            batchObject.parallelStream()
                    .collect(Collectors.groupingBy(s -> ThreadLocalRandom.current().nextInt(1, 3) > 1))
