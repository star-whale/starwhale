...
/**
 * task status change for graphing
 */
@Component
@Order(2)
public class TaskWatcherForDAG implements TaskStatusChangeWatcher {

    final DAGEditor dagEditor;

    public TaskWatcherForDAG(DAGEditor dagEditor) {
        this.dagEditor = dagEditor;
    }

    @Override
    public void onTaskStatusChange(Task task, TaskStatus newStatus) {
        dagEditor.taskStatusChange(task,newStatus);
    }
}
