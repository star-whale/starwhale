...
def iter_item():
    for i in range(10):
        yield f"image-{i}".encode(), {"label": i}
```

本例中，handler为一个generator function，Starwhale SDK根据首个yield出来的元素为非`Starwhale.Link`类型，决定构建swds-bin格式的数据集，等同于继承 `starwhale.SWDSBinBuildExecutor` 类。
