...
buid_runtime_image() {
  pushd ../../docker
  docker build -t starwhale -f Dockerfile.starwhale --build-arg PIPY_REPO_E2E=true --build-arg PORT_NEXUS=$PORT_NEXUS --build-arg LOCAL_PYPI_HOSTNAME=$IP_DOCKER_BRIDGE --build-arg SW_VERSION=$PYPI_RELEASE_VERSION .
