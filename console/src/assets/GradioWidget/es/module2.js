...
const extendBrokerImplementation = (partialBrokerImplementation) => __spreadProps(__spreadValues({}, partialBrokerImplementation), {
  connect: ({ call }) => {
    return async () => {
...
const createBroker = (brokerImplementation) => {
  ...
  return (sender) => {
    ...
    for (const [key, handler] of Object.entries(fullBrokerImplementation)) {
      functions = __spreadProps(__spreadValues({}, functions), { [key]: handler({ call, notify }) });
    }
    return __spreadValues({}, functions);
  };
};
