...
import { c as createBroker, a as addUniqueNumber, g as generateUniqueNumber } from "./module2.js";
...
const createIsSupportedPromise = (window2) => {
  if (window2 !== null && window2.BlobEvent !== void 0 && window2.MediaStream !== void 0 && (window2.MediaRecorder === void 0 || window2.MediaRecorder.isTypeSupported !== void 0)) {
...
const createNativeMediaRecorderFactory = (createInvalidModificationError2, createNotSupportedError2) => {
              ...
              setTimeout(() => {
                if (isActive && nativeMediaRecorder.state === "inactive") {
                  ...
                  if (bufferedBlobs.length > 0) {
                    const blob2 = event.data;
                    Object.defineProperty(event, "data", {
                      value: new Blob([...bufferedBlobs, blob2], { type: blob2.type })
                    });
  ...
  return (ongoingRequests, port) => {
    return (message, transferables = []) => {
      return new Promise((resolve, reject) => {
        ...
        port.postMessage(__spreadValues({ id }, message), transferables);
      });
    };
  };
};
const createRecorderAudioWorkletNodeFactory = (createListener2, createPostMessage, on2, validateState2) => {
  return (audioWorkletNodeConstructor2, context, options = {}) => {
    const audioWorkletNode = new audioWorkletNodeConstructor2(context, "recorder-audio-worklet-processor", __spreadProps(__spreadValues({}, options), {
...
const createAudioBufferConstructor = (audioBufferStore2, cacheTestResult2, createNotSupportedError2, nativeAudioBufferConstructor2, nativeOfflineAudioContextConstructor2, testNativeAudioBufferConstructorSupport, wrapAudioBufferCopyChannelMethods2, wrapAudioBufferCopyChannelMethodsOutOfBounds2) => {
  ...
  return class AudioBuffer {
    constructor(options) {
      ...
      const { length, numberOfChannels, sampleRate } = __spreadValues(__spreadValues({}, DEFAULT_OPTIONS$3), options);
...
const createAudioBufferSourceNodeConstructor = (audioNodeConstructor2, createAudioBufferSourceNodeRenderer2, createAudioParam2, createInvalidStateError2, createNativeAudioBufferSourceNode2, getNativeContext2, isNativeOfflineAudioContext2, wrapEventListener2) => {
  return class AudioBufferSourceNode extends audioNodeConstructor2 {
    constructor(context, options) {
      const nativeContext = getNativeContext2(context);
      const mergedOptions = __spreadValues(__spreadValues({}, DEFAULT_OPTIONS$2), options);
...
const createAudioWorkletNodeConstructor = (addUnrenderedAudioWorkletNode2, audioNodeConstructor2, createAudioParam2, createAudioWorkletNodeRenderer2, createNativeAudioWorkletNode2, getAudioNodeConnections2, getBackupOfflineAudioContext2, getNativeContext2, isNativeOfflineAudioContext2, nativeAudioWorkletNodeConstructor2, sanitizeAudioWorkletNodeOptions2, setActiveAudioWorkletNodeInputs2, testAudioWorkletNodeOptionsClonability2, wrapEventListener2) => {
  return class AudioWorkletNode extends audioNodeConstructor2 {
    constructor(context, name, options) {
      ...
      const nativeContext = getNativeContext2(context);
      const isOffline = isNativeOfflineAudioContext2(nativeContext);
      const mergedOptions = sanitizeAudioWorkletNodeOptions2(__spreadValues(__spreadValues({}, DEFAULT_OPTIONS$1), options));
...
const addAudioWorkletModule = isSecureContext ? createAddAudioWorkletModule(cacheTestResult, createNotSupportedError, createEvaluateSource(window$2), exposeCurrentFrameAndCurrentTime, createFetchSource(createAbortError), getNativeContext, getOrCreateBackupOfflineAudioContext, isNativeOfflineAudioContext, nativeAudioWorkletNodeConstructor, /* @__PURE__ */ new WeakMap(), /* @__PURE__ */ new WeakMap(), createTestAudioWorkletProcessorPostMessageSupport(nativeAudioWorkletNodeConstructor, nativeOfflineAudioContextConstructor), window$2) : void 0;
...
const createWebmPcmMediaRecorderFactory = (createBlobEvent2, createInvalidModificationError2, createNotSupportedError2, decodeWebMChunk2, readVariableSizeInteger2) => {
