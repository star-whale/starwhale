...
import type { State, StatefulComponentContainerProps, StateChangeType, StateReducer } from './types'
...
class StatefulListContainer extends React.Component<StatefulComponentContainerProps, State> {
    ...
    onChange = ({ oldIndex, newIndex, targetRect }: { oldIndex: number; newIndex: number; targetRect: ClientRect }) => {
        const newItemsState =
            newIndex === -1 ? arrayRemove(this.state.items, oldIndex) : arrayMove(this.state.items, oldIndex, newIndex)
        this.props.onChange({
