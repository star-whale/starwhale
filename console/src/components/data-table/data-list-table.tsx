...
import { normalizeColumns } from '../BaseTable/BaseTable.d'
...
function CellPlacement({ columnIndex, rowIndex, data, style }: any) {
    ...
    const Cell = React.useMemo(() => {
        console.log('Cell rerender', rowIndex)
        return data.columns[columnIndex].renderCell
    }, [data.textQuery, columnIndex, rowIndex])
...
function compareCellPlacement(prevProps: any, nextProps: any) {
    console.log('compareCellPlacement #########################')
        ...
        return columns.map((column, columnIndex) => {
            return (
                <CellPlacement
                    ...
                    // @ts-ignore
                    style={{
                        position: columnIndex == 0 || columnIndex == columns.length - 1 ? 'sticky' : undefined,
                        float: columnIndex == 0 ? 'left' : columnIndex == columns.length - 1 ? 'right' : undefined,
                        left: columnIndex == 0 ? 0 : undefined,
                        backgroundColor: columnIndex == 0 || columnIndex == columns.length - 1 ? 'red' : undefined,
                        ...
                        >
                            <div
                                className={css({
                                    ...
                                    boxSizing: 'border-box',
                                    display: 'flex',
                                    boxShadow: '2px 0 4px 0 #eeeeee',
                                    position:
                                        columnIndex == 0 || columnIndex == ctx.columns.length - 1
                                            ? 'sticky'
                                            : undefined,
                                    float:
                                        columnIndex == 0
                                            ? 'left'
                                            : columnIndex == ctx.columns.length - 1
                                            ? 'right'
                                            : undefined,
                                    left: columnIndex == 0 ? 0 : undefined,
                                    right: columnIndex == ctx.columns.length - 1 ? 0 : undefined,
                                })}
...
export function DataTable({
    ...
    const prev = usePrevious(handleRowMouseEnter)
