...
// Files
import { STATE_CHANGE_TYPES, KEY_STRINGS } from 'baseui/menu/constants'
import { scrollItemIntoView } from 'baseui/menu/utils'
// Types
import type {
...
> {
    ...
    internalSetState(changeType: keyof typeof STATE_CHANGE_TYPES, changes: Partial<StatefulContainerState>) {
    ...
    focusMenu = (event: FocusEvent | React.MouseEvent | KeyboardEvent) => {
        ...
        if (
            ...
            this.keyboardControlNode?.focus()
        }
    }

    unfocusMenu = () => {
        // @ADD always focus the menu
        this.internalSetState(STATE_CHANGE_TYPES.focus, { isFocused: true })
    }
...
export class StatefulFilterMenu extends React.PureComponent<StatefulMenuProps> {
    render() {
        const { overrides, ...props } = this.props
        return (
            <MenuStatefulContainer {...props}>
                {(renderProps) => <Menu {...renderProps} overrides={overrides} />}
            </MenuStatefulContainer>
        )
    }
}
