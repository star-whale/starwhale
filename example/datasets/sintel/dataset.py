...
from starwhale import Link, Image, NumpyBinary, dataset, MIMEType  # noqa: F401
...
def path_to_image(image_path:str):
    return Image(link=Link(image_path), mime_type=MIMEType.PNG,)
...
def flo_to_binary(flow_bytes:bytes):
    ...
    if 202021.25 != magic:
        ...
        return NumpyBinary(fp=data.tobytes(), shape=(h.item(), w.item(), 2), dtype=np.float32)
    ...
    for folder in json:
        ...
        for d in folder["contents"]:
            ...
            for f in d["contents"]:
                ...
                ds.append({
                    "frame0/albedo": path_to_image(file_path(PATH_ALBEDO, _dir, img_file_name)),
                    "frame0/clean": path_to_image(file_path(PATH_CLEAN, _dir, img_file_name)),
                    "frame0/final": path_to_image(file_path(PATH_FINAL, _dir, img_file_name)),
                    "frame1/albedo": path_to_image(file_path(PATH_ALBEDO, _dir, next_image(img_file_name))),
                    "frame1/clean": path_to_image(file_path(PATH_CLEAN, _dir, next_image(img_file_name))),
                    "frame1/final": path_to_image(file_path(PATH_FINAL, _dir, next_image(img_file_name))),
                    "flow_viz": path_to_image(file_path(PATH_FLOW_VIZ, _dir, img_file_name)),
                    "flow_bin": flo_to_binary(request_link_content(file_path(PATH_FLOW, _dir, flo_fn))),
                    "pix_occlusions": path_to_image(file_path(PATH_OCCLUSIONS, _dir, img_file_name)),
                    "pix_invalid": path_to_image(file_path(PATH_INVALID, _dir, img_file_name)),
                })
