...
with PILImage.open(io.BytesIO(l_img_bytes)) as img, PILImage.open(io.BytesIO(msk_img_bytes)).convert("RGBA") as msk:
    draw = ImageDraw.Draw(img)
    msk.putalpha(60)
    img.paste(msk, (0, 0), mask=msk)
    ax[0].imshow(img)

with PILImage.open(io.BytesIO(r_img_bytes)) as img, PILImage.open(io.BytesIO(msk_img_bytes)).convert("RGBA") as msk:
    draw = ImageDraw.Draw(img)
    msk.putalpha(60)
    img.paste(msk, (0, 0), mask=msk)
    ax[1].imshow(img)
